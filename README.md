...
```mermaid
flowchart TD
  A[Imagen de entrada] --> B[Detección de bordes difusa]
  B --> C[Umbralización]
  C --> D[Componentes conectados<br/>(mayor componente)]
  D --> E[Rectificación por homografía
           (gris + binaria)]
  E --> F[Extracción de celdas
           (gris + bin)]
  F --> G1[Reconocimiento HOG+kNN]
  F --> G2[OCR Tesseract (respaldo)]
  G1 --> H[Consenso + Limpieza]
  G2 --> H
  H --> I[Solucionador Sudoku (MRV+BT)]
  I --> J[Sudoku resuelto]
```
